...
1. **Cloud Authentication**:
   ...
   - Additional endpoint: `https://wyze-app-us-west-2.api.wyze.com`
...
> **Note:** The theoretical path for a local ownership app with root access is to read the SQLite databases or SharedPreferences XML files created by the official Wyze app to extract the authentication bearer token or the `enr` (encryption key) directly, bypassing the need to ping `auth-prod.api.wyze.com`. Without root scraping, the app MUST rely on standard Wyze API key authentication as implemented in go2rtc.
